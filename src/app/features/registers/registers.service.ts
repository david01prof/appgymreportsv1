...
export class RegistersService {
  ...
  public getAllMeasurements() : Observable<IMeasurement[]>{
    const queryFn = query(this._measurementCollection, orderBy('created', 'desc'));
    return collectionData(queryFn, {idField: 'id'}) as Observable<IMeasurement[]>
  }
  
  public newMeasurement(measurement: Partial<IMeasurement>): Promise<DocumentReference<DocumentData, DocumentData>>  {
